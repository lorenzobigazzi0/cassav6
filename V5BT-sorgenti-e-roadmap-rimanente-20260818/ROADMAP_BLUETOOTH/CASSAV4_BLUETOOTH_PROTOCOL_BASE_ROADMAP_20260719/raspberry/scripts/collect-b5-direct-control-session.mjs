...
const RADIO_LOCK_DIRECTORY = "/var/lib/cassav5bt-bluetooth";
...
function requireApi31StagingLocation(location, label) {
  if (
    ...
    !location.toLowerCase().includes("cassav5bt")
  ) {
    fail(
      "STAGING_REGISTRY_PATH_INVALID",
      `${label} must be an absolute isolated V5BT location`
    );
  }
...
function validateApi31RegistryInspection(value) {
  ...
  requireEqual(
    inspection.kind,
    "cassav5bt.bluetooth.device-registry",
    "STAGING_REGISTRY_INVALID",
...
async function snapshotRelevantServices() {
  ...
  const runtimeFields = unitsOutput
    ...
    .filter((fields) =>
      fields.length >= 4 &&
      (fields[0] === "bluetooth.service" || /^cassav5bt.*\.service$/u.test(fields[0]))
    );
  ...
  if (
    ...
    fail("SERVICE_PREFLIGHT_FAILED", "Competing V5BT Bluetooth services must be stopped");
  }
  const nodeUnitFile = unitFileFields.find(
    (fields) => fields[0] === "cassav5bt-bluetooth-node.service"
  );
  ...
  const enrollmentUnitFile = unitFileFields.find(
    (fields) => fields[0] === "cassav5bt-bluetooth-enrollment.service"
  );
  if (
...
export async function runApi31StagingPreflight(options, runtime = {}) {
  ...
  if (
    ...
    directReport?.v5btProductionServiceChanges !== false
  ) {
    fail(
...
async function snapshotApi31StagingServices() {
  ...
  try {
    snapshots = await Promise.all(
      ["bluetooth.service", "cassav5bt.service"].map(async (unit) => {
...
function loadPhysicalConfiguration() {
  ...
  if (
    ...
    !config.deviceRegistryPath.toLowerCase().includes("cassav5bt")
  ) {
...
function assertProvisionedRadioDirectory() {
  ...
  try {
    ...
    if (
      ...
      fail(
        "RADIO_LOCK_UNAVAILABLE",
        "The provisioned V5BT state directory must be private"
      );
    }
    if (
      process.platform === "linux" &&
      ...
      fail(
        "RADIO_LOCK_UNAVAILABLE",
        "The V5BT state directory belongs to another account"
      );
    }
    return resolved;
  } catch (error) {
    ...
    fail(
      "RADIO_LOCK_UNAVAILABLE",
      "The V5BT state directory must be provisioned before physical collection",
      1,
      { cause: error }
    );
  }
...
export async function runApi31StagingDirectControl(
  ...
  try {
    releaseRadioLock = await lockAcquirer(
      ...
      {
        busyCode: "RADIO_BUSY",
        busyMessage: "The Bluetooth adapter is already used by another V5BT harness"
      }
    );
    if (controller.signal.aborted) {
      fail("CAPTURE_ABORTED", "Physical collection was interrupted", 130);
...
function usage() {
  return [
    "V5BT B5 resumable physical-session collector",
    "",
    "Usage:",
