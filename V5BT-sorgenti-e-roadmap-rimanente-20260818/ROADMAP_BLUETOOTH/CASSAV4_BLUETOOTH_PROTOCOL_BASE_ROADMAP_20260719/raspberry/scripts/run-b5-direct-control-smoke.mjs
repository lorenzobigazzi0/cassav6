...
function applicationSnapshot(hello) {
  return {
    applicationPath: "/com/cassav5bt/gatt",
...
function parseArguments(argv) {
  ...
  if (options.selfTest) {
    ...
    if (options.registryPath === null || !path.isAbsolute(options.registryPath)) {
      fail("INVALID_ARGUMENT", "--registry must be an absolute V5BT location");
    }
    if (!options.registryPath.toLowerCase().includes("cassav5bt")) {
      fail("INVALID_ARGUMENT", "--registry must reference isolated V5BT state");
    }
  }
...
function usage() {
  return [
    "V5BT B5.7 Android-Raspberry physical direct-control smoke",
    "",
    "Usage:",
