...
export const V5BT_STATE_ROOT = "/var/lib/cassav5bt-bluetooth";
...
export function validateConfiguredStateRoot(stateRootValue) {
  if (
    ...
    stateRootValue.trim() !== "" &&
    path.resolve(stateRootValue) !== V5BT_STATE_ROOT
  ) {
    throw new Error("CASSA_BT_STATE_ROOT cannot redefine the V5BT state root");
  }
  return V5BT_STATE_ROOT;
}

export async function isolatedRegistryPath(
  registryValue,
  { stateRoot = V5BT_STATE_ROOT } = {}
) {
  ...
  if (
    ...
    throw new Error("CASSA_BT_DEVICE_REGISTRY must stay inside the V5BT state root");
  }
  ...
  if (
    ...
    throw new Error("V5BT state root must be an owned 0700 directory");
  }
  if (
    ...
    throw new Error("V5BT registry parent must be a private owned directory");
  }
  if (
    ...
    throw new Error("V5BT registry must be an owned single-link 0600 file");
  }
  ...
  if (
    ...
    throw new Error("V5BT registry path must not traverse symbolic links");
  }
  return registryPath;
}
...
export async function main() {
  ...
  if (peerTrustFlag === "1") {
    ...
    if (!isWithin(publishedPath, V5BT_STATE_ROOT)) {
      throw new Error("peer trust published path must stay in the V5BT state root");
    }
