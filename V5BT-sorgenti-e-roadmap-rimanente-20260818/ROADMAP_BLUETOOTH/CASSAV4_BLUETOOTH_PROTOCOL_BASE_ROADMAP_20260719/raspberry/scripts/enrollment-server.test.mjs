...
test("startup accepts only a canonical endpoint and the fixed V5BT root", () => {
  assert.equal(validateEndpointId("raspberry-lab-v5bt"), "raspberry-lab-v5bt");
  assert.throws(() => validateEndpointId("../invalid"));
  assert.equal(
    validateConfiguredStateRoot("/var/lib/cassav5bt-bluetooth"),
    "/var/lib/cassav5bt-bluetooth"
  );
  assert.throws(
    () => validateConfiguredStateRoot("/var/lib/cassav4-bluetooth")
  );
});

test("registry preflight requires an initialized private V5BT registry", async (t) => {
  const stateRoot = await privateDirectory(t, "cassav5bt-state-");
...
test("registry preflight rejects V4, intermediate symlink and hard-link escapes", async (t) => {
  await assert.rejects(
    isolatedRegistryPath(
      ...
      { stateRoot: "/var/lib/cassav4-bluetooth" }
    )
  );

  const stateRoot = await privateDirectory(t, "cassav5bt-state-");
  const outside = await privateDirectory(t, "cassav4-outside-");
...
test("registry preflight migrates a valid v1 file without changing credentials", async (t) => {
  const stateRoot = await privateDirectory(t, "cassav5bt-state-migration-");
  const registryPath = path.join(stateRoot, "devices.json");
  ...
  assert.equal(inspected.kind, "cassav5bt.bluetooth.device-registry");
});
...
function enrollmentRequest(version) {
  ...
  const request = {
    ...
    token: `c5e${version}_${Buffer.alloc(32, version).toString("base64url")}`,
    nodeId: NODE_ID,
