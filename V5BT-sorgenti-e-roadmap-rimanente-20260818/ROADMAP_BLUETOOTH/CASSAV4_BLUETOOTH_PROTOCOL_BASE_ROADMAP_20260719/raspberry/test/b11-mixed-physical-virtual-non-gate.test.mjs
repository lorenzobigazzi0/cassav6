  ...
  return {
    ...
    raspberry: {
      ...
      services: [
        ...
        service("cassav5bt-bluetooth-node.service"),
        service("cassav5bt-bluetooth-enrollment.service")
      ],
      registry: {
        devices: 3,
        activeDevices: 3,
...
test("file runner requires 0600 regular evidence and binds exact source bytes", async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), "v5bt-b11-mixed-test-"));
  try {
...
test("report output is atomic 0600 and rejects overwrite or symlink paths", async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), "v5bt-b11-mixed-output-"));
  const physical = bytes(makeLiveAttestation());
  const simulated = await simulatedBytes();
  try {
    const report = composeB11MixedPhysicalVirtualReport({
...
test("CLI writes the report and prints only a redacted summary", async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), "v5bt-b11-mixed-cli-"));
  try {
    const now = new Date();
