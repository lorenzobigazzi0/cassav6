...
async function testRegistry(t, options = {}) {
  const directory = await mkdtemp(path.join(tmpdir(), "cassav4-b1-registry-"));
...
test("CLI never promotes a committed token record with a mismatched pending secret", async (t) => {
  ...
  const mismatchedToken = `c5e1_${Buffer.alloc(32, 0xa5).toString("base64url")}`;
  const altered = {
    ...issued,
...
test("V5BT CLI rejects direct V4 state and hard-linked registries", async (t) => {
  const directV4 = runCli([
    "list",
    "--registry",
    "/var/lib/cassav4-bluetooth/devices.json"
  ]);
  assert.equal(directV4.status, 1);
  assert.equal(JSON.parse(directV4.stderr).code, "V4_STATE_FORBIDDEN");
