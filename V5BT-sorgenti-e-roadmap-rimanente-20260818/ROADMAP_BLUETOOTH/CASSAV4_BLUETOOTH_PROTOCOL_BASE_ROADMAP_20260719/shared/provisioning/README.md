...
contratto v1. Usa token `c5e2_`, SPKI canonico P-256 da 91 byte,
...
`CASSAV5BT-BT-ENROLLMENT-PROOF-V2` e prosegue, nell'ordine, con versione,
...
`raspberry/systemd/cassav5bt-bluetooth-enrollment.service` imposta
...
La unit crea `/var/lib/cassav5bt-bluetooth` tramite
`StateDirectory=cassav5bt-bluetooth` con modo `0700`; registry e scritture
restano confinati in quello state root V5BT.
...
`/etc/cassav5bt/cassav5bt-bluetooth-enrollment.env`. L'esempio pronto da
installare e `configs/cassav5bt-bluetooth-enrollment.env.example`: usa righe
`KEY=VALUE`, registry e state root esclusivamente V5BT, loopback e runtime
spento. Va installato come file regolare `root:root` con modo `0600`, quindi
verificato prima di un eventuale avvio Lab:

```bash
sudo install -d -o root -g cassav5bt -m 0750 /etc/cassav5bt
sudo install -o root -g root -m 0600 \
  configs/cassav5bt-bluetooth-enrollment.env.example \
  /etc/cassav5bt/cassav5bt-bluetooth-enrollment.env
...
file separato, posseduto dall'utente `cassav5bt` e con modo esatto `0600`, come
