...
export const GATT_SERVICE_UUID = "b1c4a500-7d1f-4f32-9a64-4f4b6c410001";
...
export function decodeLegacyAdvertisingData(value) {
  ...
  if (advertisingData.byteLength !== LEGACY_ADVERTISEMENT_BUDGET.totalBytes) {
    fail(
      ...
      `CASSAv4 v1 AdvData must be exactly ${LEGACY_ADVERTISEMENT_BUDGET.totalBytes} bytes`
    );
  }
  ...
  while (offset < advertisingData.byteLength) {
    ...
    if (structureType === AD_TYPE_FLAGS && flagsSeen) {
      fail("DUPLICATE_FLAGS", "CASSAv4 v1 AdvData contains duplicate Flags");
    }
    if (
      ...
      fail(
        ...
        "CASSAv4 v1 AdvData contains duplicate 128-bit Service Data"
      );
    }
    ...
    if (structureType === AD_TYPE_FLAGS) {
      ...
      if (
        ...
        fail(
          "NON_CANONICAL_FLAGS",
          "CASSAv4 v1 AdvData requires the exact Flags structure 02 01 06"
        );
      }
      ...
      if (structureLength !== serviceDataLength) {
        fail(
          ...
          "CASSAv4 v1 AdvData requires one exact 128-bit Service Data structure"
        );
      }
      ...
      if (bytesToHex(serviceUuid) !== bytesToHex(expectedUuid)) {
        fail(
          ...
          "CASSAv4 v1 AdvData contains an unexpected Service Data UUID"
        );
      }
      ...
      fail(
        ...
        `CASSAv4 v1 AdvData contains unexpected AD type 0x${structureType
          .toString(16)
          .padStart(2, "0")}`
      );
    }
  ...
  if (structureCount !== 2) {
    fail(
      ...
      "CASSAv4 v1 AdvData must contain exactly two AD structures"
    );
  }
  if (!flagsSeen) {
    fail("MISSING_FLAGS", "CASSAv4 v1 AdvData is missing Flags");
  }
  if (servicePayload === undefined) {
    fail(
      "MISSING_SERVICE_DATA",
      "CASSAv4 v1 AdvData is missing 128-bit Service Data"
    );
  }
