...
node scripts/collect-b4-physical-device.mjs `
  --init `
  --state D:\cassav2\.v5bt-private\b4-device-gate-state.json
```
...
```powershell
node scripts/run-v5bt-b4-android-continuity-monitor.mjs `
  --adb C:\path\to\adb.exe `
  --serial SERIAL_ADB_TARGET `
  --android-user-id 0 `
  --capture-run-id UUID_CAPTURE `
  --collector-state D:\cassav2\.v5bt-private\b4-device-gate-state.json `
...
```powershell
node scripts/run-v5bt-b4-raspberry-continuity-monitor.mjs `
  --host RASPBERRY_TEST `
  --user admin `
...
node scripts/collect-b4-physical-device.mjs `
  --finalize `
  --state D:\cassav2\.v5bt-private\b4-device-gate-state.json `
  --manifest D:\cassav2\.v5bt-private\b4-manifest.json
```
...
```bash
install -d -m 700 /var/lib/cassav5bt-bluetooth/b4-evidence
```
...
npm run gate:b4-ten-device -- \
  --manifest /var/lib/cassav5bt-bluetooth/b4-evidence/manifest.json \
  --registry /var/lib/cassav5bt-bluetooth/devices.json \
  --output ../reports/physical/v5bt-b4-4-ten-device-gate.json
```
