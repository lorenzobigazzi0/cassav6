...
function gattReporter(overrides = {}) {
  ...
  return {
    schemaVersion: 4,
    source: "V5BT_ANDROID_DIRECT_CONTROL_LAB",
    labBuild: true,
...
test("private JSON publication is 0600, atomic and never overwrites", () => {
  const directory = temporaryDirectory("v5bt-b5-monitor-private-");
  try {
...
test("private JSON rejects permissive modes, hard links and every symlink", () => {
  const directory = temporaryDirectory("v5bt-b5-monitor-private-invalid-");
  try {
...
test("Android paired publication recovers failures and protects its exact journal", () => {
  const directory = temporaryDirectory("v5bt-b5-monitor-publication-");
  try {
...
test("Android paired publication recovers after both commits before journal cleanup", () => {
  const directory = temporaryDirectory("v5bt-b5-monitor-post-commit-");
  try {
    const options = {
...
test("self-test is offline, pending and filesystem-neutral", () => {
  const directory = temporaryDirectory("v5bt-b5-monitor-self-test-");
  try {
...
test("module import has no CLI or filesystem side effects", () => {
  const directory = temporaryDirectory("v5bt-b5-monitor-import-");
  try {
