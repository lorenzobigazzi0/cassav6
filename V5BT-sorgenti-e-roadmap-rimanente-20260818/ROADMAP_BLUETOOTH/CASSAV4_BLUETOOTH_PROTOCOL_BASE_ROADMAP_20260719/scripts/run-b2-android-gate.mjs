...
function parseStatus(raw) {
  ...
  if (
    ...
    status.source !== 'V5BT_ANDROID_DISCOVERY_LAB' ||
    status.labBuild !== true ||
    status.diagnosticsEnabled !== true
  ) {
    throw statusValidationError(
      'LAB_MARKERS_INVALID',
      'diagnostic status is not a V5BT Lab report'
    );
  }
...
function buildDiagnosticSummary(
  ...
  return {
    ...
    source: cooldownPilot
      ...
      : 'V5BT_B2_ANDROID_HANDHELD_PAIR_DIAGNOSTIC',
...
function buildSummary(
  ...
  return {
    schemaVersion: REPORT_SCHEMA_VERSION,
    source: 'V5BT_B2_ANDROID_GATE_HARNESS',
    certificationMatrixBinding: ADVANCED_CERTIFICATION_TARGETS_BINDING,
...
async function runSelfTest() {
  ...
  const evidenceFixtureDirectory = fs.mkdtempSync(
    path.join(os.tmpdir(), 'v5bt-b2-evidence-')
  );
  ...
  return {
    schemaVersion: REPORT_SCHEMA_VERSION,
    source: 'V5BT_B2_ANDROID_GATE_HARNESS_SELF_TEST',
    certificationMatrixBinding: ADVANCED_CERTIFICATION_TARGETS_BINDING,
    result: 'PASS',
    tests,
    coverage: {
