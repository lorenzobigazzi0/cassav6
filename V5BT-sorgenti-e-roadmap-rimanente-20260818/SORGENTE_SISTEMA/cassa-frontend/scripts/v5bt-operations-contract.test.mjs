...
import { V5BT_BATTERY_NOTIFICATION_INTERVAL_MS } from "./v5bt-operations-gates.mjs";
...
test("le fixture restano uniche finche il pool dispone di tavoli", () => {
  ...
  assert.deepEqual(
    availableV5btFixtureTables(tables, { reservedTableIds }),
    [{ id: "table-2" }],
  );
...
test("il riuso fixture riparte solo a pool esaurito e preserva tavoli estranei", () => {
  ...
  assert.deepEqual(
    availableV5btFixtureTables(tables, {
      reservedTableIds,
      allowReuse: true,
    }),
    tables,
...
test("il ciclo fixture copre le 651 comande richieste dal profilo lungo", () => {
  ...
  for (let index = 0; index < 651; index += 1) {
    const available = availableV5btFixtureTables(tables, {
      reservedTableIds,
      allowReuse: true,
    });
...
test("il launcher operativo fissa il profilo massimo 25x5 e le durate full/smoke/micro", async () => {
  const launcher = await source("run-v5bt-operations-30.mjs");

  assert.equal(V5BT_MAX_HANDHELDS, 25);
  assert.equal(V5BT_MAX_STATIONS, 5);
  assert.match(launcher, /LOADTEST_PROFILE:\s*"v5bt-operations-30"/);
  ...
  assert.match(launcher, /(?:FULL|V5BT_FULL)_ACTIONS_PER_DEVICE\s*=\s*200/);
  assert.match(launcher, /(?:SMOKE|V5BT_SMOKE)_ACTIONS_PER_DEVICE\s*=\s*40/);
  assert.match(launcher, /V5BT_MICRO_ACTIONS_PER_DEVICE\s*=\s*10/);
  assert.match(launcher, /const mode = micro \? "micro"/);
  assert.match(
    launcher,
    /LOADTEST_V5BT_ACTIONS_PER_DEVICE:\s*String\([\s\S]*actionsPerDevice[\s\S]*\)/,
  );
  const runner = await source("loadtest-full-capacity.mjs");
  assert.match(runner, /V5BT_ACTIONS_PER_DEVICE < 10/);
  assert.doesNotMatch(runner, /V5BT_ACTIONS_PER_DEVICE < 40/);
});
...
test("il launcher attesta il preflight pressione host e blocca solo dopo il dry-run", async () => {
  ...
  assert.match(launcher, /evaluateV5btHostPressure\(\{/);
...
test("il launcher operativo fissa e verifica il profilo concorrente L1-L4", async () => {
  ...
  assert.match(launcher, /requestedDiagnosticLaneMatrix !== V5BT_DIAGNOSTIC_LANE_MATRIX/);
  assert.match(
    launcher,
  ...
  assert.match(launcher, /V5BT_CERTIFIED_PAYMENT_LANE_CONCURRENCY\s*=\s*2/);
  assert.match(launcher, /V5BT_DIAGNOSTIC_PAYMENT_LANE_CONCURRENCY\s*=\s*3/);
  assert.match(launcher, /V5BT_CERTIFIED_AUTO_PRINT_OWNER_INTERVAL_MS\s*=\s*25/);
  assert.match(launcher, /V5BT_DIAGNOSTIC_AUTO_PRINT_OWNER_INTERVAL_MS\s*=\s*100/);
  assert.match(
    launcher,
  ...
  assert.match(launcher, /LOADTEST_V5BT_EVIDENCE_CLASS:\s*v5btOperationsEvidenceClass/);
  assert.match(
    launcher,
    /LOADTEST_V5BT_PROMOTION_ELIGIBILITY:\s*v5btOperationsPromotionEligibility/,
  );
  ...
  assert.match(runner, /process\.env\.LOADTEST_V5BT_EVIDENCE_CLASS/);
  assert.match(runner, /process\.env\.LOADTEST_V5BT_PROMOTION_ELIGIBILITY/);
  assert.match(runner, /rawV5btDiagnosticPaymentLaneConcurrency === "3"/);
  assert.match(runner, /v5btOperationsEvidenceClass === "NON_GATE"/);
  assert.match(runner, /v5btOperationsPromotionEligibility === "NON_PROMOTABLE"/);
  ...
  assert.match(runner, /v5bt\.operations\.auto_print_owner_only/);
  const autoPrintOwnerGate = await source("v5bt-auto-print-owner-gate.mjs");
  ...
  const stationStateMarkerGate = await source("v5bt-station-state-marker-gate.mjs");
  assert.match(
  ...
  assert.match(runner, /v5btOperationsDiagnosticPaymentLaneConcurrency:/);
  assert.match(runner, /v5btOperationsDiagnostic:/);
  assert.match(runner, /printLaneConcurrency,/);
  assert.match(runner, /printSpoolAutoPrintOwnerIntervalMs,/);
  assert.match(runner, /v5btOperationsDiagnosticAutoPrintOwnerIntervalMs:/);
...
test("il dry-run classifica l'override payment 3 come NON_GATE e rifiuta altri valori", () => {
  const launcherPath = path.join(scriptDir, "run-v5bt-operations-30.mjs");
  const cleanEnv = { ...process.env };
  delete cleanEnv.LOADTEST_V5BT_DIAGNOSTIC_PAYMENT_LANE_CONCURRENCY;
  delete cleanEnv.LOADTEST_V5BT_DIAGNOSTIC_LANE_MATRIX;
  delete cleanEnv.LOADTEST_V5BT_DIAGNOSTIC_AUTO_PRINT_OWNER_INTERVAL_MS;
  delete cleanEnv.LOADTEST_V5BT_DIAGNOSTIC_STATION_STATE_MARKER_LOCK_SKIP;
  delete cleanEnv.LOADTEST_STATION_STATE_MARKER_LOCK_SKIP;
  delete cleanEnv.LOADTEST_V5BT_DIAGNOSTIC_STATION_STATE_LAST_WRITE_COALESCE;
  ...
  const diagnostic = spawnSync(process.execPath, [launcherPath, "--dry-run"], {
    ...
    env: {
      ...cleanEnv,
      LOADTEST_V5BT_DIAGNOSTIC_PAYMENT_LANE_CONCURRENCY: "3",
    },
  });
  ...
  for (const invalidValue of ["1", "2", "03", "4"]) {
    const invalid = spawnSync(process.execPath, [launcherPath, "--dry-run"], {
      ...
      env: {
        ...cleanEnv,
        LOADTEST_V5BT_DIAGNOSTIC_PAYMENT_LANE_CONCURRENCY: invalidValue,
      },
    });
...
test("il dry-run fissa owner auto-print a 25 ms e ammette solo il canary NON_GATE da 100 ms", () => {
  ...
  const diagnostic = spawnSync(process.execPath, [launcherPath, "--dry-run"], {
    ...
    env: {
      ...cleanEnv,
      LOADTEST_V5BT_DIAGNOSTIC_AUTO_PRINT_OWNER_INTERVAL_MS: "100",
    },
  });
  assert.equal(diagnostic.status, 0, diagnostic.stderr);
  const diagnosticManifest = JSON.parse(diagnostic.stdout);
  ...
  for (const invalidValue of ["10", "25", "0100", "101"]) {
    const invalid = spawnSync(process.execPath, [launcherPath, "--dry-run"], {
      ...
      env: {
        ...cleanEnv,
        LOADTEST_V5BT_DIAGNOSTIC_AUTO_PRINT_OWNER_INTERVAL_MS: invalidValue,
      },
    });
    assert.notEqual(invalid.status, 0);
    assert.match(
...
test("il dry-run mantiene OFF lo skip lock marker station-state e ammette solo il canary ON NON_GATE", () => {
  ...
  for (const invalidValue of ["0", "true", "01", "ON", "2"]) {
    const invalid = spawnSync(process.execPath, [launcherPath, "--dry-run"], {
      ...
      env: {
        ...cleanEnv,
        LOADTEST_V5BT_DIAGNOSTIC_STATION_STATE_MARKER_LOCK_SKIP: invalidValue,
      },
    });
    assert.notEqual(invalid.status, 0);
    assert.match(
...
test("il dry-run mantiene OFF il coalescing lastWriteAt station-state e ammette solo il canary ON NON_GATE", () => {
  ...
  for (const invalidValue of ["0", "true", "01", "ON", "2"]) {
    const invalid = spawnSync(process.execPath, [launcherPath, "--dry-run"], {
      ...
      env: {
        ...cleanEnv,
        LOADTEST_V5BT_DIAGNOSTIC_STATION_STATE_LAST_WRITE_COALESCE: invalidValue,
      },
    });
    assert.notEqual(invalid.status, 0);
    assert.match(
...
test("lo spool stampa V5BT e per-run, validato e rimosso senza usare quello storico", async () => {
  ...
  assert.equal(resolvePrintSpoolDir("", "/tmp/v5bt-backend"), "/tmp/v5bt-backend/.print-spool");
  assert.equal(
    resolvePrintSpoolDir("/tmp/v5bt-report/runtime/print-spool", "/tmp/v5bt-backend"),
    "/tmp/v5bt-report/runtime/print-spool",
  );
  assert.throws(() => resolvePrintSpoolDir("relative", "/tmp/v5bt-backend"));
  assert.throws(() => resolvePrintSpoolDir("/", "/tmp/v5bt-backend"));
  assert.throws(() => resolvePrintSpoolDir("/tmp", "/tmp/v5bt-backend"));
...
test("lo scheduler e il load test bloccano 3 secondi per azione e 7-8 per comanda", async () => {
  ...
  assert.equal(V5BT_DEVICE_ACTION_INTERVAL_MS, 3_000);
  assert.equal(V5BT_COMMAND_INTERVAL_MIN_MS, 7_000);
  assert.equal(V5BT_COMMAND_INTERVAL_MAX_MS, 8_000);
  assert.match(runner, /actionIntervalMs:\s*V5BT_DEVICE_ACTION_INTERVAL_MS/);
  assert.match(runner, /commandIntervalMinMs:\s*V5BT_COMMAND_INTERVAL_MIN_MS/);
  assert.match(runner, /commandIntervalMaxMs:\s*V5BT_COMMAND_INTERVAL_MAX_MS/);
  assert.match(runner, /v5bt\.operations\.mobile_action_cadence_3s/);
  assert.match(runner, /v5bt\.operations\.command_cadence_7_8s/);
  assert.match(runner, /v5bt\.operations\.early_dispatch_action_burst/);
...
test("report e manifest attestano scheduler dispatch v2 e batteria ogni 120 secondi", async () => {
  ...
  assert.equal(V5BT_OPERATIONS_SCHEDULER_CONTRACT_VERSION, 2);
  assert.equal(V5BT_BATTERY_NOTIFICATION_INTERVAL_MS, 120_000);
  assert.match(launcher, /schedulerContractVersion:\s*V5BT_OPERATIONS_SCHEDULER_CONTRACT_VERSION/);
  assert.match(launcher, /batteryNotificationIntervalMs:\s*V5BT_BATTERY_NOTIFICATION_INTERVAL_MS/);
  assert.match(runner, /v5btSchedulerContractVersion:\s*V5BT_OPERATIONS_PROFILE/);
  assert.match(runner, /v5btOperationsStage:\s*V5BT_OPERATIONS_PROFILE/);
  assert.match(runner, /batteryNotificationIntervalMs:\s*REALISTIC_LOAD_PROFILE/);
  assert.match(runner, /MOCK_BATTERY_CHANGE_INTERVAL_MS:\s*String\([\s\S]{0,100}V5BT_BATTERY_NOTIFICATION_INTERVAL_MS/);
  assert.match(runner, /notificationIntervalMs ===[\s\S]{0,100}V5BT_BATTERY_NOTIFICATION_INTERVAL_MS/);
...
test("il budget GUI V5BT misura soltanto la finestra azioni dopo le fixture", async () => {
  const runner = await source("loadtest-full-capacity.mjs");
  const profile = between(
    runner,
    "async function runP5EnduranceProfile",
  ...
  const fixturesAt = profile.indexOf("await prepareV5btOperationFixtures");
...
test("il dispatcher mobile copre storno, spostamenti, BANCO e trasferimenti", async () => {
  ...
  assert.match(dispatcher, /Azione V5BT non gestita/);
});
...
test("report e gate rendono bloccanti quota, copertura, successo e cadenza", async () => {
  ...
  const gate = between(
    runner,
    "function recordV5btOperationsProfileFailures",
    "async function runP5EnduranceProfile",
  );

  assert.match(
    gate,
    /requiredMobileTypes\s*=\s*\["order\.create", \.\.\.V5BT_MOBILE_OPERATION_TYPES\]/,
  );
  ...
  assert.match(gate, /v5bt\.operations\.total_started/);
  assert.match(gate, /v5bt\.operations\.total_completed/);
  assert.match(gate, /v5bt\.operations\.action_coverage_missing/);
  assert.match(gate, /v5bt\.operations\.action_without_success/);
  assert.match(gate, /v5bt\.operations\.mobile_cadence/);
  assert.match(gate, /v5bt\.operations\.command_cadence/);
  assert.match(gate, /v5bt\.operations\.early_action_burst/);
  assert.match(gate, /v5bt\.operations\.early_dispatch_action_burst/);
  assert.match(gate, /v5bt\.operations\.global_backpressure/);
  assert.match(gate, /v5bt\.operations\.device_backpressure/);
  assert.match(gate, /v5bt\.operations\.gui_request_amplification/);
  assert.match(gate, /v5bt\.operations\.action_p95/);
  assert.match(gate, /v5bt\.operations\.command_p95/);
  assert.match(runner, /v5btOperationsProfile,/);
  assert.match(launcher, /completedReport\?\.v5btOperationsProfile/);
  ...
  assert.match(launcher, /completedReport\?\.config\?\.v5btSchedulerContractVersion/);
  assert.match(launcher, /completedReport\?\.config\?\.v5btOperationsStage/);
});

test("il report V5BT rende bloccante l'attribuzione latenza schema v1", async () => {
  const runner = await source("loadtest-full-capacity.mjs");
  assert.match(runner, /import \{ buildV5btLatencyAttribution \}/);
  assert.match(runner, /v5bt\.operations\.latency_attribution_complete/);
  assert.match(runner, /v5bt\.operations\.latency_attribution_incomplete/);
...
test("i retry della stessa comanda mantengono una chiave idempotente stabile", async () => {
  ...
  assert.match(createAction, /runV5btOrderCreateRetry\(\{/);
  assert.match(createAction, /maxAttempts:\s*5/);
  assert.match(createAction, /idempotencyKey:\s*stableKey/);
  assert.match(createAction, /acquireV5btOrderCreateTable\(\{/);
  assert.match(createAction, /timeoutMs:\s*V5BT_ORDER_CREATE_TABLE_WAIT_MS/);
  assert.match(createAction, /createTableReservations\.delete\(tableId\)/);
  assert.ok(
    createAction.indexOf("const idempotencyKey") <
      createAction.indexOf("acquireV5btOrderCreateTable"),
  );
});
...
test("le fixture 25+5 sono admin-created, drenate e fuori dal conteggio esatto", async () => {
  ...
  const fixtures = between(
    runner,
    "async function prepareV5btOperationFixtures",
  ...
  assert.match(fixtures, /V5BT_READY_ORDER_ACTION_TYPES\.has\(actionType\)/);
  ...
  assert.match(
    profile,
    /const fixtureSummary = operationsProfile[\s\S]+prepareV5btOperationFixtures\(devices, context\)/,
  );
  assert.ok(
  ...
  assert.match(
    runner,
    /fixtureSummary:\s*summary\.v5btOperationsProfile\.fixtureSummary/,
  );
...
test("le operazioni distruttive usano prerequisiti dedicati e diagnostica redatta", async () => {
  ...
  const selector = between(
    runner,
    "function selectV5btOperationOrder",
    "async function runV5btOperationsHandheldAction",
  );
  const dispatcher = between(
    runner,
  ...
  const diagnostics = between(
    runner,
    "function v5btOperationFailureDetails",
    "function selectV5btOperationOrder",
  );

  assert.match(selector, /const readyOrders = Array\.isArray\(device\.readyOrders\)/);
  assert.match(selector, /V5BT_READY_ORDER_ACTION_TYPES\.has\(actionType\)/);
  ...
  assert.match(selector, /async function waitForV5btReadyOrder/);
  ...
  assert.match(
    dispatcher,
    /V5BT_READY_ORDER_ACTION_TYPES\.has\(actionType\)[\s\S]+await waitForV5btReadyOrder/,
  );
...
test("i conflitti transitori usano retry limitati e mantengono proprietario e revisione correnti", async () => {
  const runner = await source("loadtest-full-capacity.mjs");
  const tableAcquisition = await source("v5bt-order-table-acquisition.mjs");
  ...
  assert.match(tableMove, /event:\s*"v5bt_table_move_revision_retry"/);
  ...
  assert.match(fixtures, /ensureV5btOrderTableCapacity\(\{/);
  assert.match(
    fixtures,
    /minimumPerHandheld:\s*V5BT_RUNTIME_LIMITS\.maxInFlightPerDevice \+ 1/,
  );
  assert.match(fixtures, /excludedTableIds:\s*excludedFixtureTableIds/);
  assert.match(createOrderAction, /acquireV5btOrderCreateTable\(\{/);
  assert.match(createOrderAction, /timeoutMs:\s*V5BT_ORDER_CREATE_TABLE_WAIT_MS/);
  assert.match(
    createOrderAction,
  ...
  assert.match(tableAcquisition, /V5btOrderTableAvailabilityTimeoutError/);
  ...
  assert.match(runner, /filterV5btStationWorkflowCandidates/);
  assert.match(runner, /claimV5btStationWorkflowTarget\(target, reservedOrderIds\)/);
...
test("l'override prezzo mobile ritenta solo TABLE_LOCKED con payload e identita stabili", async () => {
  const runner = await source("loadtest-full-capacity.mjs");
  const retry = await source("v5bt-mobile-action-retry.mjs");
  ...
  assert.match(retry, /V5BT_MOBILE_TABLE_LOCK_MAX_ATTEMPTS = 2/);
  ...
  assert.match(
    retry,
    /Math\.min\([\s\S]+V5BT_MOBILE_TABLE_LOCK_MAX_ATTEMPTS/,
  );
  ...
  assert.match(priceOverride, /runV5btMobileBusinessActionRetry\(\{/);
...
test("le prenotazioni operative riservano slot non confliggenti per tavolo", async () => {
  ...
  const reservationSlot = between(
    runner,
    "function nextV5btReservationAt",
    "async function runV5btOperationsHandheldAction",
  );
  const dispatcher = between(
    runner,
  ...
  assert.match(
    dispatcher,
    /reservationFlow\(session, table, \{[\s\S]+nextV5btReservationAt\(context, table\)/,
  );
