...
import {
  ...
  V6OrderTableAvailabilityTimeoutError,
  acquireV6OrderCreateTable,
} from "./v6-order-table-acquisition.mjs";
...
test("attende una contesa locale e riserva atomicamente il primo tavolo liberato", async () => {
  ...
  const acquired = await acquireV6OrderCreateTable({
...
test("il timeout e' tipizzato e non altera le prenotazioni esistenti", async () => {
  ...
  await assert.rejects(
    ...
    (error) => {
      assert.ok(error instanceof V6OrderTableAvailabilityTimeoutError);
      assert.equal(error.code, V6_ORDER_TABLE_AVAILABILITY_TIMEOUT);
