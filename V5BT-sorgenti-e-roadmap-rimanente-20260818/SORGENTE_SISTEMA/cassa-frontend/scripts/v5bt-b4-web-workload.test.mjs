...
import { V5BT_MOBILE_OPERATION_TYPES } from "./v5bt-operations-scheduler.mjs";
import {
...
test("il piano copre 8 Palmare, 160 azioni, 64 ordini e tutti i 35 tipi", () => {
  ...
  assert.equal(
    ...
    V5BT_MOBILE_OPERATION_TYPES.length + 1,
  );
...
test("il contratto DOM recupera overlay e richiede linee canoniche dopo refresh", async () => {
  const source = await fs.readFile(new URL("./v5bt-b4-web-workload.mjs", import.meta.url), "utf8");
...
test("gli screenshot privati usano file esclusivi 0600 e rifiutano symlink", async (t) => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "v5bt-workload-test-"));
