...
import {
  V6_COMMAND_AVERAGE_INTERVAL_MS,
  V6_COMMAND_CADENCE_JITTER_MS,
  V6_DEVICE_ACTION_INTERVAL_MS,
  V6_MAX_HANDHELDS,
  V6_MAX_STATIONS,
  V6_MOBILE_OPERATION_TYPES,
  calculateV6OperationsSchedule,
  countV6Commands,
  isV6CommandOrdinal,
  runV6OperationsSchedule,
  summarizeV6OperationsCadence,
  v6MobileActionType,
  validateV6OperationsConfig,
...
test("il profilo ammette una sola categoria di device ma non zero device", () => {
  ...
  const handheldOnlySchedule = calculateV6OperationsSchedule({
    handheldCount: 8,
    stationCount: 0,
    actionsPerDevice: 20,
  });
...
test("la cadenza comande alterna 6 e 9 secondi per una media di 7,5", () => {
  ...
  assert.equal(countV6Commands(200), 80);
  const starts = ordinals.map(
    (ordinal) => (ordinal - 1) * V6_DEVICE_ACTION_INTERVAL_MS,
  );
  ...
  const fullStarts = Array.from({ length: 200 }, (_, index) => index + 1)
    .filter(isV6CommandOrdinal)
    .map((ordinal) => (ordinal - 1) * V6_DEVICE_ACTION_INTERVAL_MS);
  ...
  assert.ok(Math.abs(averageMs - V6_COMMAND_AVERAGE_INTERVAL_MS) < 25);
});
...
test("il catalogo operativo viene interamente pianificato", () => {
  ...
  for (let device = 0; device < 25; device += 1) {
    const paymentActions = Array.from({ length: 10 }, (_, index) =>
      v6MobileActionType(device, index + 1),
...
test("il riepilogo distingue cadenza azioni e cadenza comande", () => {
  ...
  for (let ordinal = 1; ordinal <= 20; ordinal += 1) {
    samples.push({
      ...
      kind: "handheld",
      ordinal,
      actionType: v6MobileActionType(0, ordinal),
      startedAt: (ordinal - 1) * 3_000,
    });
  }
  const summary = summarizeV6OperationsCadence(samples);
...
test("il riepilogo valuta il dispatch reale e conserva il piano come diagnostica", () => {
  const summary = summarizeV6OperationsCadence([
    {
      deviceId: "mobile-1",
      kind: "handheld",
...
test("il riepilogo distingue il jitter normale da una raffica anticipata", () => {
  const jitter = summarizeV6OperationsCadence([
    {
      deviceId: "mobile-1",
      kind: "handheld",
      actionType: "layout.get",
  ...
  const burst = summarizeV6OperationsCadence([
    {
      deviceId: "mobile-1",
      kind: "handheld",
      actionType: "layout.get",
...
test("la cadenza comande ammette solo 10 ms di jitter del timer", () => {
  ...
  assert.equal(V6_COMMAND_CADENCE_JITTER_MS, 10);
...
test("lo scheduler mantiene la quota senza sovrapporre azioni veloci dello stesso device", async () => {
  ...
  const result = await runV6OperationsSchedule({
    devices: [
...
test("lo scheduler mantiene la cadenza di dispatch anche con una risposta ancora pendente", async () => {
  ...
  const starts = [];
  const scheduled = runV6OperationsSchedule({
    devices: [
      { id: "mobile-1", kind: "handheld", index: 0 },
      { id: "station-1", kind: "station", index: 0 },
    ],
