...
import {
  V6_LATENCY_ATTRIBUTION_SCHEMA_VERSION,
  buildV6LatencyAttribution,
} from "./v6-latency-attribution.mjs";
...
test("il builder produce attribution schema v1 completa e redatta", () => {
  const attribution = buildV6LatencyAttribution(completeEvidence());

  assert.equal(attribution.schemaVersion, V6_LATENCY_ATTRIBUTION_SCHEMA_VERSION);
  assert.equal(attribution.schemaVersion, 1);
  assert.equal(attribution.kind, "v6-latency-attribution");
...
test("latenze alte restano dati e non introducono soglie arbitrarie", () => {
  ...
  const attribution = buildV6LatencyAttribution(evidence);
  assert.equal(attribution.status, "COMPLETE");
  assert.equal(
...
test("istogrammi e contatori malformati non vengono normalizzati in un PASS", () => {
  ...
  let attribution = buildV6LatencyAttribution(invalidHistogram);
  ...
  attribution = buildV6LatencyAttribution(invalidCounter);
  assert.equal(attribution.categories.proxyOwner.status, "INCOMPLETE");
  assert.ok(
