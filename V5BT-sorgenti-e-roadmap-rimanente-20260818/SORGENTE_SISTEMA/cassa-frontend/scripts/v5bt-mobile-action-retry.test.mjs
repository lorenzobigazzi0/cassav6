...
import {
  ...
  V5BT_MOBILE_TABLE_LOCK_RETRY_DELAY_MS,
  isV5btTransientMobileTableLock,
  runV5btMobileBusinessActionRetry,
} from "./v5bt-mobile-action-retry.mjs";
...
test("un TABLE_LOCKED transitorio ritenta una volta con identita logica invariata", async () => {
  ...
  const result = await runV5btMobileBusinessActionRetry(
    baseOptions({
  ...
  assert.deepEqual(waits, [V5BT_MOBILE_TABLE_LOCK_RETRY_DELAY_MS]);
...
test("un TABLE_LOCKED persistente resta fallimento dopo il limite stretto", async () => {
  ...
  assert.equal(attempts, V5BT_MOBILE_TABLE_LOCK_MAX_ATTEMPTS);
...
test("identita logica, chiave e callback mancanti sono rifiutate", async () => {
  ...
  await assert.rejects(runV5btMobileBusinessActionRetry(baseOptions()), /attempt/);
});
