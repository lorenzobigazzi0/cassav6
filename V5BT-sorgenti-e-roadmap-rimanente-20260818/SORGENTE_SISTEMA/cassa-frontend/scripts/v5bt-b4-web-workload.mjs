...
const DOM_EXECUTION_ATTESTATION = Symbol("v5bt-b4-built-in-dom-execution");
...
export function resolveB4WebWorkloadActionType({ deviceIndex = 0, ordinal }) {
  return v5btMobileActionType(deviceIndex, ordinal);
}

function resolveDomScenario(ordinal) {
  if (isV5btCommandOrdinal(ordinal)) return "ORDER_CREATE";
  return (
    {
      2: "TABLE_SEARCH",
      4: "BEST_SELLER",
...
function reportPasses(report) {
  return (
    ...
    report.execution.plannedOperationTypes === V5BT_MOBILE_OPERATION_TYPES.length + 1 &&
...
export function buildB4WebWorkloadReport(metrics = {}) {
  ...
  const report = {
    ...
    profile: {
      ...
      batteryIntervalConfiguredMs: V5BT_BATTERY_NOTIFICATION_INTERVAL_MS,
...
export async function runB4WebWorkload(
  ...
  try {
    ...
    scheduleResult = await runV5btOperationsSchedule({
