...
import {
  V6_HOST_PRESSURE_MAX_LOAD_PER_CPU,
  V6_HOST_PRESSURE_THRESHOLDS_BYTES,
...
test("la matrice usa le soglie conservative richieste per ogni profilo", () => {
  assert.deepEqual(V6_HOST_PRESSURE_THRESHOLDS_BYTES, {
...
test("una lettura esattamente in soglia autorizza lo smoke", () => {
  const result = evaluateV6HostPressure({
    ...QUIET_HOST,
    platform: "linux",
    mode: "smoke",
...
test("memoria insufficiente blocca lo smoke e swap insufficiente blocca il full", () => {
  const smoke = evaluateV6HostPressure({
    ...QUIET_HOST,
    platform: "linux",
    mode: "smoke",
  ...
  const full = evaluateV6HostPressure({
    ...QUIET_HOST,
    platform: "linux",
    mode: "full",
    memAvailableBytes: 4 * GIB,
...
test("il carico scheduler e normalizzato per CPU e blocca oltre soglia", () => {
  const atThreshold = evaluateV6HostPressure({
    platform: "linux",
    mode: "micro",
    memAvailableBytes: 1 * GIB,
    swapFreeBytes: 512 * MIB,
    loadAverage1m: V6_HOST_PRESSURE_MAX_LOAD_PER_CPU * 4,
    logicalCpuCount: 4,
  });
  const overloaded = evaluateV6HostPressure({
    platform: "linux",
    mode: "micro",
    memAvailableBytes: 1 * GIB,
    swapFreeBytes: 512 * MIB,
    loadAverage1m: V6_HOST_PRESSURE_MAX_LOAD_PER_CPU * 4 + 0.01,
    logicalCpuCount: 4,
  });
...
test("soltanto il valore esplicito 1 abilita e attesta l'override", () => {
  ...
  const accepted = evaluateV6HostPressure({ ...base, overrideValue: "1" });
  const rejected = evaluateV6HostPressure({ ...base, overrideValue: "true" });
...
test("letture mancanti sono bloccanti e non Linux e dichiarato non applicabile", () => {
  const missing = evaluateV6HostPressure({
    platform: "linux",
    mode: "full",
  });
  const otherPlatform = evaluateV6HostPressure({
    platform: "win32",
    mode: "full",
  });
