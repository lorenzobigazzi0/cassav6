...
export const V5BT_ORDER_TABLE_AVAILABILITY_TIMEOUT =
  "V5BT_ORDER_TABLE_AVAILABILITY_TIMEOUT";

const normalizedTableId = (table) => String(table?.id ?? "").trim();

export class V5btOrderTableAvailabilityTimeoutError extends Error {
  constructor({ timeoutMs, authorizedTableCount, scanCount }) {
    ...
    this.name = "V5btOrderTableAvailabilityTimeoutError";
    this.code = V5BT_ORDER_TABLE_AVAILABILITY_TIMEOUT;
...
export async function acquireV5btOrderCreateTable({
  authorizedTables,
  ...
  while (true) {
    ...
    if (remainingMs <= 0) {
      throw new V5btOrderTableAvailabilityTimeoutError({
        timeoutMs: timeout,
