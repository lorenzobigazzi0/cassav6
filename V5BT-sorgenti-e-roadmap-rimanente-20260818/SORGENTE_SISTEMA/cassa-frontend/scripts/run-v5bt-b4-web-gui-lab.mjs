...
} from "./v5bt-b4-web-workload.mjs";
import { V5BT_BATTERY_NOTIFICATION_INTERVAL_MS } from "./v5bt-operations-gates.mjs";
import { V5BT_DEVICE_ACTION_INTERVAL_MS } from "./v5bt-operations-scheduler.mjs";
  ...
  return Object.freeze(
    Array.from({ length: count }, (_, index) => {
      ...
      return Object.freeze({
        ...
        deviceUuid: `v5bt-b4-web-slot-${String(slot).padStart(2, "0")}`,
...
export function validatePublicReport(report) {
  ...
  requireExactValue(report.product, "V5BT");
...
async function findChromeExecutable() {
  const candidates = [
    process.env.V5BT_B4_WEB_GUI_CHROME,
...
export async function startIsolatedStationHeartbeat(
  backendOrigin,
  {
    ...
    onFailure = () => undefined,
    deviceUuid = `v5bt-b4-station-${crypto.randomBytes(12).toString("hex")}`,
  } = {},
) {
...
async function loginWebPalmare(browser, frontendUrl, entry, privateDir, stopLab) {
  ...
  await page.evaluate((slot) => {
    document.title = `V5BT Palmare Web ${String(slot).padStart(2, "0")}`;
  }, entry.slot);
    ...
    JSON.stringify(
      {
        ...
        workload: {
          ...
          batteryIntervalMs: V5BT_BATTERY_NOTIFICATION_INTERVAL_MS,
        },
      },
      null,
      2,
...
function usage() {
  console.error(
    "Usage: node scripts/run-v5bt-b4-web-gui-lab.mjs --start|--status|--pilot|--workload|--stop|--dry-run",
  );
}
