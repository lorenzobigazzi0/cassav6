...
} from "./run-v5bt-b4-web-gui-lab.mjs";
...
async function fixture(t) {
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "v5bt-b4-web-gui-"));
...
test("private directory is validated before its mode is changed", async (t) => {
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "v5bt-private-dir-"));
  t.after(() => fs.rm(directory, { recursive: true, force: true }));
...
test("private JSON is read from a safe single file descriptor", async (t) => {
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "v5bt-private-json-"));
  t.after(() => fs.rm(directory, { recursive: true, force: true }));
...
test("private JSON rejects wrong mode, hardlinks and files over 64 KiB", async (t) => {
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "v5bt-private-limits-"));
  t.after(() => fs.rm(directory, { recursive: true, force: true }));
