...
export const V5BT_OPERATIONS_READINESS_VERSION = "1.0.0";
export const V5BT_SCHEDULER_CONTRACT_VERSION = 2;
...
export class V5btOperationsReadinessError extends Error {
  constructor(code, message, options = undefined) {
    super(message, options);
    this.name = "V5btOperationsReadinessError";
    this.code = code;
  }
}

function fail(code, message, options = undefined) {
  throw new V5btOperationsReadinessError(code, message, options);
}
...
function stageShell(stage, status, sourceReportSha256, checks, observed, timing = null) {
  return {
    public: {
      ...
      sourceReportSha256,
      expected: {
        handhelds: V5BT_MAX_HANDHELDS,
        stations: V5BT_MAX_STATIONS,
...
function evaluateCurrentStage(stage, report, digest) {
  ...
  const profile = report.v5btOperationsProfile;
  const checks = initialChecks("FAIL");
  const configVersion = safeInteger(config?.v5btSchedulerContractVersion);
  const profileVersion = safeInteger(profile?.schedulerContractVersion);
  const configStage = config?.v5btOperationsStage;
  ...
  if (markersMissing || configVersion < V5BT_SCHEDULER_CONTRACT_VERSION) {
    return stageShell(
      stage,
      "STALE",
      digest,
  ...
  checks.contractVersion =
    configVersion === V5BT_SCHEDULER_CONTRACT_VERSION &&
    profileVersion === V5BT_SCHEDULER_CONTRACT_VERSION
      ? "PASS"
      : "FAIL";
  checks.stageBinding =
  ...
  checks.profileBinding =
    config?.profile === "v5bt-operations-30" &&
    ...
    config?.v5btOperationsEvidenceClass === "QUALIFYING_PROFILE" &&
    config?.v5btOperationsPromotionEligibility === "READINESS_ELIGIBLE" &&
    config?.v5btOperationsDiagnosticPaymentLaneConcurrency === null &&
    config?.v5btOperationsDiagnostic === false &&
  ...
  const actionsPerDevice = safeInteger(config?.v5btOperationsActionsPerDevice);
  ...
  checks.deviceTopology =
    handhelds === V5BT_MAX_HANDHELDS &&
    stations === V5BT_MAX_STATIONS &&
    profile?.handheldCount === V5BT_MAX_HANDHELDS &&
    profile?.stationCount === V5BT_MAX_STATIONS &&
    profile?.deviceCount === V5BT_MAX_HANDHELDS + V5BT_MAX_STATIONS &&
    devices.length === V5BT_MAX_HANDHELDS + V5BT_MAX_STATIONS &&
    handheldDeviceCount === V5BT_MAX_HANDHELDS &&
    stationDeviceCount === V5BT_MAX_STATIONS
      ? "PASS"
      : "FAIL";
  checks.workloadQuota =
    actionsPerDevice === spec.actionsPerDevice &&
    config?.v5btOperationsTotalActions === spec.totalActions &&
  ...
  checks.mobileActionCadence =
    ...
    mobileAverage !== null &&
    mobileAverage >= V5BT_DEVICE_ACTION_INTERVAL_MS - 5 &&
    mobileAverage <= V5BT_DEVICE_ACTION_INTERVAL_MS + 300
      ? "PASS"
      : "FAIL";
  checks.commandCadence =
    cadence?.commandCadenceOk === true &&
    commandAverage !== null &&
    commandAverage >= V5BT_COMMAND_INTERVAL_MIN_MS &&
    commandAverage <= V5BT_COMMAND_INTERVAL_MAX_MS
      ? "PASS"
      : "FAIL";
  checks.latency =
    finiteNumber(profile?.actionLatencyMs?.p95ms) !== null &&
    profile.actionLatencyMs.p95ms <= V5BT_ACTION_P95_MAX_MS &&
    finiteNumber(profile?.commandLatencyMs?.p95ms) !== null &&
    profile.commandLatencyMs.p95ms <= V5BT_COMMAND_P95_MAX_MS
      ? "PASS"
      : "FAIL";
  ...
  checks.batteryCadence =
    config?.batteryNotificationIntervalMs === V5BT_BATTERY_NOTIFICATION_INTERVAL_MS &&
    battery?.ok === true &&
    battery?.body?.notificationIntervalMs === V5BT_BATTERY_NOTIFICATION_INTERVAL_MS &&
    batteryDeviceCount === V5BT_MAX_HANDHELDS
      ? "PASS"
      : "FAIL";
  ...
  checks.persistence =
    ...
    persistedDevices === V5BT_MAX_HANDHELDS
      ? "PASS"
      : "FAIL";
...
export function evaluateV5btOperationsReadiness({
  certificationMatrix,
  ...
  const base = {
    schemaVersion: 1,
    harnessVersion: V5BT_OPERATIONS_READINESS_VERSION,
    product: "V5BT",
...
export function parseV5btOperationsReadinessReceipt(raw) {
  let receipt;
  try {
  ...
  if (
    ...
    receipt.harnessVersion !== V5BT_OPERATIONS_READINESS_VERSION ||
    receipt.product !== "V5BT" ||
...
function safeReadFile(location, maximumBytes, { missingAllowed = false } = {}) {
  ...
  try {
    ...
    if (error instanceof V5btOperationsReadinessError) throw error;
...
export function writeImmutableV5btOperationsReadinessReceipt(outputPath, receipt) {
...
export function validV5btOperationsStageFixture(
  stage,
  { startedAt, endedAt } = {},
) {
  ...
  const devices = Array.from(
    { length: V5BT_MAX_HANDHELDS + V5BT_MAX_STATIONS },
    (_, deviceIndex) => ({
      id: `private-device-${deviceIndex + 1}`,
      kind: deviceIndex < V5BT_MAX_HANDHELDS ? "handheld" : "station",
  ...
  return {
    ...
    backendPid: 1234,
    config: {
      profile: "v5bt-operations-30",
      handHeldCount: V5BT_MAX_HANDHELDS,
      stationCount: V5BT_MAX_STATIONS,
      v5btOperationsActionsPerDevice: spec.actionsPerDevice,
      v5btOperationsTotalActions: spec.totalActions,
      v5btSchedulerContractVersion: V5BT_SCHEDULER_CONTRACT_VERSION,
      v5btOperationsStage: stage,
      ...
      v5btOperationsDiagnosticPaymentLaneConcurrency: null,
      v5btOperationsDiagnostic: false,
    ...
    v5btOperationsProfile: {
      schedulerContractVersion: V5BT_SCHEDULER_CONTRACT_VERSION,
      stage,
      handheldCount: V5BT_MAX_HANDHELDS,
      stationCount: V5BT_MAX_STATIONS,
      deviceCount: V5BT_MAX_HANDHELDS + V5BT_MAX_STATIONS,
      actionsPerDevice: spec.actionsPerDevice,
      ...
      cadence: {
        ...
        mobileActionAverageGapMs: V5BT_DEVICE_ACTION_INTERVAL_MS,
      ...
      devicesMeetingPersistedOrderTarget: V5BT_MAX_HANDHELDS,
      runtimeGate: {
        ok: true,
    ...
    mockIoMetrics: {
      battery: {
        ok: true,
        body: {
          notificationIntervalMs: V5BT_BATTERY_NOTIFICATION_INTERVAL_MS,
          devices: Array.from({ length: V5BT_MAX_HANDHELDS }, () => ({})),
        },
      },
    },
    cleanup: {
...
function usage() {
  return [
    "V5BT operations load readiness receipt (NON_GATE)",
    "",
    "node scripts/v5bt-operations-readiness.mjs --matrix MATRIX.json \\",
...
function safeFailure(error) {
  return {
    ...
    failure: {
      code:
        error instanceof V5btOperationsReadinessError
          ? error.code
...
export async function main(argv = process.argv.slice(2)) {
  try {
    ...
    const receipt = evaluateV5btOperationsReadiness({
    ...
    writeImmutableV5btOperationsReadinessReceipt(options.output, receipt);
