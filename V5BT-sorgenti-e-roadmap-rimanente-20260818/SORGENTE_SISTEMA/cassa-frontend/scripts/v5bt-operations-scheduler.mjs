export const V5BT_MAX_HANDHELDS = 25;
export const V5BT_MAX_STATIONS = 5;
export const V5BT_OPERATIONS_SCHEDULER_CONTRACT_VERSION = 2;
export const V5BT_DEVICE_ACTION_INTERVAL_MS = 3_000;
export const V5BT_COMMAND_AVERAGE_INTERVAL_MS = 7_500;
export const V5BT_COMMAND_INTERVAL_MIN_MS = 7_000;
export const V5BT_COMMAND_INTERVAL_MAX_MS = 8_000;
export const V5BT_COMMAND_CADENCE_JITTER_MS = 10;

export const V5BT_MOBILE_OPERATION_TYPES = Object.freeze([
  "order.sync.ready",
  "payment.amount_free",
  "table.move",
  "order.correct",
...
export function validateV5btOperationsConfig({
  handheldCount,
  stationCount,
  actionsPerDevice,
  ...
  if (handhelds > V5BT_MAX_HANDHELDS) {
    throw new Error(`handheldCount supera il massimo ${V5BT_MAX_HANDHELDS}.`);
  }
  if (stations > V5BT_MAX_STATIONS) {
    throw new Error(`stationCount supera il massimo ${V5BT_MAX_STATIONS}.`);
  }
  if (intervalMs !== V5BT_DEVICE_ACTION_INTERVAL_MS) {
    throw new Error(
      `actionIntervalMs deve restare ${V5BT_DEVICE_ACTION_INTERVAL_MS} ms.`,
    );
  }
...
export function isV5btCommandOrdinal(ordinal) {
  const safeOrdinal = positiveInteger(ordinal, "ordinal");
  const position = (safeOrdinal - 1) % 5;
  return position === 0 || position === 2;
}

export function countV5btCommands(actionsPerDevice) {
...
function nonCommandOrdinal(ordinal) {
  ...
  for (let current = 1; current <= ordinal; current += 1) {
    if (!isV5btCommandOrdinal(current)) count += 1;
  }
  return count;
}

export function v5btMobileActionType(deviceIndex, ordinal) {
  const safeDeviceIndex = Math.max(0, Math.trunc(Number(deviceIndex) || 0));
  const safeOrdinal = positiveInteger(ordinal, "ordinal");
  if (isV5btCommandOrdinal(safeOrdinal)) return "order.create";
  const operationOrdinal = nonCommandOrdinal(safeOrdinal);
  const offset =
    (safeDeviceIndex * 11 + operationOrdinal - 1) %
    V5BT_MOBILE_OPERATION_TYPES.length;
  return V5BT_MOBILE_OPERATION_TYPES[offset];
}

export function v5btStationActionType(deviceIndex, ordinal) {
  const types = [
    "station.heartbeat",
    "station.orders.poll",
    "station.order.prep",
...
export function calculateV5btOperationsSchedule(config) {
  const validated = validateV5btOperationsConfig(config);
  ...
  return Object.freeze({
    ...
    commandsPerHandheld: countV5btCommands(validated.actionsPerDevice),
  });
}
...
export function summarizeV5btOperationsCadence(
  samples,
  actionIntervalMs = V5BT_DEVICE_ACTION_INTERVAL_MS,
) {
    ...
    mobileDevices.every(
      (device) =>
        ...
        device.commandAverageGapMs >=
          V5BT_COMMAND_INTERVAL_MIN_MS - V5BT_COMMAND_CADENCE_JITTER_MS &&
        device.commandAverageGapMs <=
          V5BT_COMMAND_INTERVAL_MAX_MS + V5BT_COMMAND_CADENCE_JITTER_MS,
    );
  return {
    cadenceBasis: "dispatch",
    targetActionIntervalMs: actionIntervalMs,
    targetCommandAverageIntervalMs: V5BT_COMMAND_AVERAGE_INTERVAL_MS,
    commandCadenceJitterMs: V5BT_COMMAND_CADENCE_JITTER_MS,
...
export async function runV5btOperationsSchedule({
  devices,
  actionsPerDevice,
  actionIntervalMs = V5BT_DEVICE_ACTION_INTERVAL_MS,
  runAction,
  ...
  if (!Array.isArray(devices) || devices.length === 0) {
    throw new Error("Scheduler V5BT: almeno un device e obbligatorio.");
  }
  if (typeof runAction !== "function") {
    throw new Error("Scheduler V5BT: runAction deve essere una funzione.");
  }
  if (typeof isActionSuccessful !== "function") {
    throw new Error(
      "Scheduler V5BT: isActionSuccessful deve essere una funzione.",
    );
  }
  ...
  try {
    while (totalStarted < schedule.totalActions) {
      ...
      if (!device) {
        ...
        throw new Error(
          "Scheduler V5BT bloccato prima di avere pianificato tutte le azioni.",
        );
      }
  ...
  return {
    ...
    cadence: summarizeV5btOperationsCadence(samples, actionIntervalMs),
