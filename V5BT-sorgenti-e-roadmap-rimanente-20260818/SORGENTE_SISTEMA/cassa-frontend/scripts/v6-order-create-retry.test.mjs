...
import { runV6OrderCreateRetry } from "./v6-order-create-retry.mjs";
...
test("TABLE_LOCKED e 428 riacquisiscono il lock con la stessa chiave idempotente", async () => {
  ...
  const result = await runV6OrderCreateRetry({
...
test("chiave o callback mancanti sono rifiutate prima del primo tentativo", async () => {
  await assert.rejects(
    runV6OrderCreateRetry({ attempt: async () => ({ status: 200 }) }),
    /idempotencyKey stabile/,
  );
  await assert.rejects(
    runV6OrderCreateRetry({ idempotencyKey: "logical-order-4" }),
