export function createSmartTransactionWriteModel(deps = {}) {
  const {
    HttpError,
  ...
  async function consumeBeachEntry(payload) {
  ...
  async function rechargeCustomer(payload) {
    const customerId = String(payload.customerId ?? "").trim();
  ...
  return { consumeBeachEntry, rechargeCustomer };
}
