...
const foundationSmokePath = path.resolve(
  "scripts/postgresql-migration/mig020-foundation-smoke.mjs",
);
...
test("le migration foundation PostgreSQL restano ordinate per responsabilita", async () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      { version: "010", name: "identity_users_groups" },
    ],
  );
...
test("lo smoke foundation usa il contratto idempotenza corrente", async () => {
  const source = await fs.readFile(foundationSmokePath, "utf8");
  assert.match(source, /app_meta_usage: true/);
  assert.match(source, /repeat\('a', 64\), 'processing', now\(\) \+ interval '15 minutes'/);
  assert.match(source, /SET status = 'completed'.*completed_at = now\(\)/);
  assert.match(source, /await client\.query\("BEGIN"\)/);
  assert.match(source, /await client\.query\("COMMIT"\)/);
  assert.match(source, /await client\.query\("ROLLBACK"\)/);
  assert.doesNotMatch(source, /'sha256:probe'|'PROCESSING'|'SUCCEEDED'/);
});
