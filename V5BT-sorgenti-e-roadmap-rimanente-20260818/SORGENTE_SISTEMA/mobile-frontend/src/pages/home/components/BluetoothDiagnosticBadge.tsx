...
import {
  ...
  type BluetoothConnectivitySnapshot,
  type BluetoothConnectivityState,
} from "../../../app/runtime/bluetoothDiagnosticConnectivity";

type BadgePresentation = {
  label: string;
  tone: "active" | "attention" | "inactive" | "pending";
};

const PRESENTATION_BY_STATE: Record<BluetoothConnectivityState, BadgePresentation> = {
  ...
  DIRECT_SERVER: { label: "Diagnostica Bluetooth: server diretto attivo", tone: "active" },
  PEER_CONNECTED: {
    label: "Diagnostica Bluetooth: collegamento peer rilevato",
    tone: "active",
  },
  DEGRADED: { label: "Diagnostica Bluetooth: servizio degradato", tone: "attention" },
  BACKOFF: {
...
export function BluetoothDiagnosticBadge() {
  const [snapshot, setSnapshot] = useState<BluetoothConnectivitySnapshot | null>(null);
