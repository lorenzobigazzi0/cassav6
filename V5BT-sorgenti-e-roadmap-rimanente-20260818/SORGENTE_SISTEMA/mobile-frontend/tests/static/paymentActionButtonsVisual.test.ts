...
describe("payment action buttons visual contract", () => {
  ...
  it("mantiene uno slide-to coerente per pagamento, liberazione e fondo cassa", () => {
    const wizard = readSource("src/pages/home/tables/components/TablePaymentWizard.tsx");
    // La conferma a scorrimento del tavolo vive nel suo dialogo estratto.
    const freeDialog = readSource("src/pages/home/tables/components/TableFreeConfirmDialog.tsx");
    ...
    expect(freeDialog).toContain("table-detail-free-slide");
    expect(freeDialog).toContain('aria-label="Scorri per liberare il tavolo"');
