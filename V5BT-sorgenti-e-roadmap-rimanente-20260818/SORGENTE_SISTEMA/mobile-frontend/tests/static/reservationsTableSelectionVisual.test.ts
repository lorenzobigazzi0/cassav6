...
describe("reservation table selection visual shell", () => {
  ...
  it("chiude il dettaglio tavolo dopo conferma prenotazione e mantiene badge leggibili", () => {
    ...
    expect(tile).toContain('import bookingsIconSrc from "../../../../assets/icons/bookings.png"');
    expect(tile).toContain('import personIconSrc from "../../../../assets/icons/profile.png"');
    expect(tile).not.toContain("pax");
    expect(tile).not.toContain("Pren.");
    expect(tile).toContain("aria-label={`${table.covers} coperti`}");
  });
});
