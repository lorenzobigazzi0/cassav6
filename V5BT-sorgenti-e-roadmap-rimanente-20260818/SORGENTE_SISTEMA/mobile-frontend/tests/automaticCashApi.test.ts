...
describe("automaticCash API client", () => {
  it("uses only backend automatic-cash endpoints", async () => {
    ...
    await getCashMovementWithdrawalAvailability();
    expectLastRequest(fetch, "/api/automatic-cash/cash-movements/withdrawal-availability", "GET");

    await getAutomaticCashMovementState("cashmov-1");
    expectLastRequest(fetch, "/api/automatic-cash/cash-movements/cashmov-1/state", "GET");
    ...
    await prepareAutomaticCashMovement("cashmov-1");
    expectLastRequest(fetch, "/api/automatic-cash/cash-movements/cashmov-1/prepare", "POST", {
    ...
    await completeAutomaticCashMovement("cashmov-1", { awaitingReport: true });
    expectLastRequest(fetch, "/api/automatic-cash/cash-movements/cashmov-1/complete", "POST", {
      movementId: "cashmov-1",
      awaitingReport: true,
    });

    await printAutomaticCashMovementReport("cashmov-1", {
      clientRequestId: "cash-report-cashmov-1-initial",
      reprint: false,
    });
    expectLastRequest(fetch, "/api/automatic-cash/cash-movements/cashmov-1/print", "POST", {
      movementId: "cashmov-1",
      clientRequestId: "cash-report-cashmov-1-initial",
      reprint: false,
    });
