...
2026-07-24 offline operational-configuration update: the Palmare keeps a user/activity-scoped
IndexedDB snapshot of available rooms, the integration table layout, every room menu, and the
reservation window. `useOfflineConfigurationSync` refreshes that snapshot after login, on network
and realtime recovery, on focus/settings changes, and every 60 seconds while online. React Query
runs domain query functions in `networkMode: "always"`, so a cold start without connectivity still
executes the repository fallbacks instead of leaving queries paused. Menu, room, table, and
reservation screens therefore continue from the last complete local configuration and replace it
incrementally when the server becomes reachable again.

Ordinary idempotent mutations use the persistent offline outbox; table layout/order mutations keep
their existing domain queue. Synthetic `202 X-Palmare-Offline-Queued` responses are treated as an
accepted local operation, and reservation snapshots are updated immediately so the UI never rolls
back an action that is already waiting for replay. Payments, fiscal operations, automatic-cash
settlement, printing, refunds, and other irreversible hardware operations remain deliberately
outside the generic queue. A replay rejected by the backend is retained as `conflict` or `failed`
for operator-visible reconciliation instead of being deleted silently.
...
replaces the local order ID in both memory and the persisted snapshot. The synchronous
`mobile:session-ending` boundary clears all in-memory room tables and integration/layout
fingerprints, preventing a second user in the same room from inheriting the previous operator's
optimistic state; persistent integration actions remain owner/activity/device scoped.
...
Automatic-cash administration (configuration, minimum reserve, and denomination-combination JSON)
and POS statistics are not Palmare settings. They belong only to the administrative frontend at
`/impostazioni`. The Palmare retains only the operational payment overview, whose resident provider
updates in the background before the route is opened.
...
Additional fiscal authorization alignment on 2026-07-17:

- `src/domain/payments/fiscalOutcome.ts` is the shared display-state owner for issued, voided,
  failed, pending, and missing fiscal outcomes.
- `src/domain/payments/fiscalAuthorization.ts` centralizes the client capability check and mirrors
  the backend rule that an admin owns every known POS permission, including `fiscal_operations`.
  The backend remains authoritative for final authorization and device eligibility.
- Analytics print-mode transitions consume that capability instead of exposing the fiscal mode to
  an unauthorized session. The former Palmare-specific manual offline action manager has been
  removed; fiscal retry safety is now owned by backend/gateway reconciliation.

Additional authoritative fiscal verification update on 2026-07-17:

- The cassa backend exposes `POST /api/reports/payment-movement/fiscal/verify` and derives payment,
  receipt, fiscal device, and idempotency key from persisted state.
- Automatic and manual issue paths verify `/api/fiscal/receipt/verify` before every retry. A
  terminal gateway result reconciles local state; only authoritative `NOT_FOUND` permits a write.
- The same rule applies to fiscal void. A missing gateway verification endpoint is never treated
...
- The source build and typecheck pass, and the live V5BT server serves the updated chunks with a
  healthy MySQL-backed API. The complete frontend suite still has 11 unrelated pre-existing
  failures; all focused best-seller tests pass.
- Palmare Advanced `1.0.26` (`versionCode 27`) was rebuilt with the B5.5 lab flags, passed Android
  unit tests and lint, and was installed in place on the first physical Palmare.
...
- Palmare Advanced `1.0.27` (`versionCode 28`) embeds the verified login bundle, passed all 149
  Android unit tests plus lint, and was installed in place on the first physical Palmare.
...
- The focused web regression set passes 51/51 tests in both source trees. Palmare Advanced
  `1.0.28` (`versionCode 29`) embeds the rebuilt frontend and passes all 160 Android unit tests.
...
Additional empty-table detail UI update on 2026-08-28:

- The free-table setup actions are explicit uppercase `OCCUPA` and `PRENOTA` tabs with distinct
  green and blue visual identities. The anagraphic labels are uppercase, and `TELEFONO` plus
  `COPERTI` use a stable two-thirds/one-third row.
- The docked order-history bar is mounted only after the table reaches the seated state.
- Manual allergy/intolerance input now uses an explicit add action. Each accepted entry becomes a
  table-scoped removable card, visually distinct from the predefined HACCP allergen cards, while
  the existing comma-separated backend contract remains unchanged.
- Manual allergy/intolerance removal now requires an explicit `ANNULLA`/`CONFERMA` alert dialog.
  The free-table occupation confirmation is a larger centered uppercase orange action.
- Table tiles replace the textual `pax` and `Pren.` abbreviations with the existing person and
  booking icon assets, preserving the numeric covers count and reservation time as accessible text.
- The empty-table detail now also exposes the full-width `LIBERA` action and reuses the same
  slide-to-confirm dialog as occupied or reserved tables. The occupied-duration badge replaces the
  `Seduto da` prefix with an hourglass while preserving the live relative duration.
- Reserved-table detail places the uppercase `ARRIVATI` action beside uppercase `LIBERA` in the
  docked bottom action bar, keeping both actions equally sized and immediately reachable.
