...
import com.sentrapa.webkiosk.bluetooth.BluetoothEnrollmentConfig
import com.sentrapa.webkiosk.bluetooth.BluetoothEnrollmentCoordinator
...
class AlwaysOnService : Service() {
    ...
    override fun onCreate() {
        ...
        activeService = this
        batteryReporter = BatteryReporter(applicationContext)
        ...
        notificationPoller = NativeNotificationPoller(applicationContext) { events, alertNew ->
            if (hasAuthenticatedSession()) {
                notificationCoordinator.syncSnapshot(events, alertNew)
            }
        }
    ...
    override fun onDestroy() {
        ...
        if (::batteryReporter.isInitialized) batteryReporter.close()
    ...
    private fun refreshRuntimeState(): Boolean {
        ...
        if (!sessionActive) notificationCoordinator.reset()
        batteryReporter.updateUrl(savedUrl)
        batteryReporter.updateDeviceIdentifier(KioskPreferences.getBatteryDeviceIdentifier(this))
    ...
    private fun stopForClearedSession() {
        ...
        if (::notificationPoller.isInitialized) {
            notificationPoller.update(savedUrl, NotificationClientContext())
        }
        if (::backgroundRadioReceiver.isInitialized) {
            backgroundRadioReceiver.update(savedUrl, NotificationClientContext())
        }
        if (::notificationCoordinator.isInitialized) notificationCoordinator.reset()
        if (::audioPlayer.isInitialized) audioPlayer.stop()
    ...
    companion object {
        ...
        private const val ACTION_ENQUEUE_NOTIFICATION =
            "com.sentrapa.webkiosk.action.ENQUEUE_NOTIFICATION"
        private const val ACTION_CLEAR_NATIVE_ALERTS =
            "com.sentrapa.webkiosk.action.CLEAR_NATIVE_ALERTS"
        private const val ACTION_REFRESH_BLUETOOTH =
            "com.sentrapa.webkiosk.action.REFRESH_BLUETOOTH"
        private const val ACTION_REFRESH_SESSION =
            "com.sentrapa.webkiosk.action.REFRESH_SESSION"
        ...
        fun start(context: Context) {
            ...
            if (
                !shouldStartAuthenticatedRuntime(
                    KioskPreferences.getSavedUrl(context),
                    sessionContext
                )
            ) return
        ...
        fun enqueueNotification(
            ...
            val appContext = context.applicationContext
            if (!KioskPreferences.hasAuthenticatedNotificationSession(appContext)) {
                return false
            }
        ...
        fun refreshSession(context: Context) {
            ...
            if (
                !shouldStartAuthenticatedRuntime(
                    KioskPreferences.getSavedUrl(appContext),
                    sessionContext
                )
            ) {
            ...
            BluetoothFailoverService.start(appContext)
            startServiceIntent(
                appContext,
                Intent(appContext, AlwaysOnService::class.java)
