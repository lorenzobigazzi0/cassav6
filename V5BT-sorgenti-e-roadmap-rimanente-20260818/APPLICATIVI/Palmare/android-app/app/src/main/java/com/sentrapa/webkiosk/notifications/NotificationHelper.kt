...
import com.sentrapa.webkiosk.AppForegroundState
import com.sentrapa.webkiosk.KioskPreferences
import com.sentrapa.webkiosk.MainActivity
import com.sentrapa.webkiosk.NativeNotificationGroup
import com.sentrapa.webkiosk.NativeNotificationSnapshot
import com.sentrapa.webkiosk.NotificationTone
import com.sentrapa.webkiosk.R
import java.util.concurrent.atomic.AtomicLong

object NotificationHelper {
    private const val CHANNEL_ALERT = "notification_alert_silent_v5"
    ...
    fun renderBackgroundSnapshot(
        ...
        if (
            ...
            clearDeliveredNotifications(context)
            return
        }
    ...
    fun vibrateForTone(context: Context, tone: NotificationTone) {
        if (!KioskPreferences.hasAuthenticatedNotificationSession(context)) return
    ...
    fun cancelActiveVibration(context: Context) {
        val appContext = context.applicationContext
        synchronized(alertDeliveryLock) {
            runCatching {
                ...
                Log.w(TAG, "Unable to cancel native notification vibration: ${error.message}")
            }
        }
    }
    ...
    fun clearDeliveredNotifications(context: Context) {
        ...
        synchronized(alertDeliveryLock) {
            ...
            val sessionActive = KioskPreferences.hasAuthenticatedNotificationSession(appContext)
            if (
                sessionActive &&
                previousClearAt > 0L &&
    ...
    private fun defaultTitle(tone: NotificationTone): String = when (tone) {
        ...
        NotificationTone.HANDHELD_RING -> "Chiamate palmare"
    ...
    private val alertDeliveryLock = Any()
}
