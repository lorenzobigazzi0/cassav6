...
private const val MOBILE_WEB_PATH = "/mobile"
private const val MOBILE_ASSET_ROOT = "mobile"

/**
 * Serves the bundled frontend while preserving the configured HTTPS origin.
 * Requests outside /mobile stay on the network, so API, SSE and WebSocket
 * traffic keep the same origin and contracts used by the hosted application.
 */
internal class PalmareWebAppAssets(context: Context) {
    ...
    fun shouldIntercept(requestUrl: Uri, configuredUrl: String?): WebResourceResponse? {
        ...
        val stream = try {
            ...
            Log.w("PalmareAssets", "Asset locale non leggibile: $assetPath", error)
            return null
        }
...
internal fun resolveBundledAssetPath(encodedPath: String): String? {
    ...
    if (path != MOBILE_WEB_PATH && !path.startsWith("$MOBILE_WEB_PATH/")) return null

    var relative = path.removePrefix(MOBILE_WEB_PATH).trimStart('/')
    ...
    return "$MOBILE_ASSET_ROOT/$relative"
}
