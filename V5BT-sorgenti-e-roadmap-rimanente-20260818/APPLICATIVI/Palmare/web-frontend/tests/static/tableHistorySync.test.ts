...
describe("table history sync", () => {
  it("riapplica le comande quando il layout tavoli cambia e mostra lo storico anche su tavolo libero", () => {
    ...
    expect(detailPanel).not.toContain("{!isFree && hasAnyOrder && (");
  });
  ...
  it("mostra il numero comanda nello storico invece del dettaglio articoli", () => {
    ...
    expect(detailPanel).not.toContain('<div className="table-history-order-title">{order.title}</div>');
  ...
  it("riallinea il dettaglio comanda aperto quando lo storico tavolo viene aggiornato", () => {
    ...
    expect(detailPanel).toContain("table.orderHistory.find((order) => order.id === selectedHistoryOrder.id)");
