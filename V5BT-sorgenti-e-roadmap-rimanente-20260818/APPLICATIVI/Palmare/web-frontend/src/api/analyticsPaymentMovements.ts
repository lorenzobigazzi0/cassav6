...
export type AnalyticsMovementFetchOptions = {
  requireLive?: boolean;
};

export type AnalyticsFiscalRequestIdentity = {
  requestId?: string;
...
export async function fetchAnalyticsPaymentMovements(
  ...
  signal?: AbortSignal,
  options: AnalyticsMovementFetchOptions = {}
  ...
  if (options.requireLive === true && response.headers?.get?.("X-Palmare-Offline-Cache") === "1") {
    throw new Error("Verifica live non disponibile: il backend non risponde.");
  }
...
async function requestAnalyticsFiscalAction(
  ...
  identity: AnalyticsFiscalRequestIdentity = {}
): Promise<AnalyticsFiscalReceipt> {
  if (!auth.token || !auth.userId || !auth.deviceUuid) {
    throw new Error("Sessione non disponibile.");
  }
  const normalizedMovementId = normalizeAnalyticsValue(movementId);
  if (!normalizedMovementId) {
    throw new Error("Movimento pagamento non valido.");
  }
  const response = await apiFetch(path, {
    ...
    headers: {
      ...
      "X-Device-Uuid": auth.deviceUuid,
      "X-User-Id": auth.userId,
      ...(normalizeAnalyticsValue(identity.requestId)
        ? { "X-Command-Request-Id": normalizeAnalyticsValue(identity.requestId) }
        : {}),
      ...(normalizeAnalyticsValue(identity.idempotencyKey)
        ? { "X-Idempotency-Key": normalizeAnalyticsValue(identity.idempotencyKey) }
        : {}),
    },
    body: JSON.stringify({
      token: auth.token,
      ...
      receiptId: normalizeAnalyticsValue(identity.receiptId || receiptId),
      ...(normalizeAnalyticsValue(identity.requestId)
        ? { requestId: normalizeAnalyticsValue(identity.requestId) }
        : {}),
      ...(normalizeAnalyticsValue(identity.idempotencyKey)
        ? { idempotencyKey: normalizeAnalyticsValue(identity.idempotencyKey) }
        : {}),
      ...extra,
    }),
  });
...
export const issueAnalyticsFiscalMovement = (
  ...
  requestAnalyticsFiscalAction(
    auth,
    record.paymentId,
    normalizeAnalyticsValue(record.raw?.fiscalReceiptId),
    FISCAL_ISSUE_PATH
  );

export const issueAnalyticsFiscalPaymentById = (
  auth: AnalyticsSessionContext,
  movementId: string,
  identity: AnalyticsFiscalRequestIdentity = {}
) =>
  requestAnalyticsFiscalAction(
    auth,
    movementId,
    normalizeAnalyticsValue(identity.receiptId),
    FISCAL_ISSUE_PATH,
    {},
    identity
  );
