...
const PLAYBACK_JITTER_BUFFER_MS = 120;
const PLAYBACK_REBUFFER_MS = 160;
const PLAYBACK_UNDERRUN_CONCEALMENT_MS = 24;
const PLAYBACK_RESUME_FADE_MS = 6;
    ...
    const creation = (async () => {
      ...
      const node = new AudioWorkletNode(context, "radio-playback-processor", {
        ...
        processorOptions: {
          ...
          concealmentSamples: Math.round(
            (context.sampleRate * PLAYBACK_UNDERRUN_CONCEALMENT_MS) / 1000
          ),
          resumeFadeSamples: Math.round((context.sampleRate * PLAYBACK_RESUME_FADE_MS) / 1000),
        },
      });
