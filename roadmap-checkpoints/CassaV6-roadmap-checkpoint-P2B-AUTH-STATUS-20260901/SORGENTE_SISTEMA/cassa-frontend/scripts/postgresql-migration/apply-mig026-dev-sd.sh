...
if [[ "${MIG026_ALLOW_DEV_APPLY:-}" != "1" ]]; then
  echo "Impostare MIG026_ALLOW_DEV_APPLY=1 per aggiornare il database DEV." >&2
  exit 2
fi
...
if [[ "${POSTGRES_DATABASE:-}" != "cassav6" ]]; then
  echo "MIG-026 DEV apply consentito soltanto sul database cassav6." >&2
  exit 4
fi
