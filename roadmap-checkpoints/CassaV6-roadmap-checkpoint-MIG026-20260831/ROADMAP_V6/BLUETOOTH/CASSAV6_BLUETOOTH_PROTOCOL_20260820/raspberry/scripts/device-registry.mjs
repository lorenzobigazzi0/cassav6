...
function usage() {
  return `CASSA V6 Bluetooth B1 device registry (offline administrative CLI)
...
const ENROLLMENT_TOKEN_PATTERN =
  /^c6e[12]_[A-Za-z0-9_-]{42}[AEIMQUYcgkosw048]$/;
const ALIAS_KEY_PATTERN = /^[A-Za-z0-9_-]{42}[AEIMQUYcgkosw048]$/;
const FORBIDDEN_LEGACY_STATE_ROOTS = Object.freeze([
  "/var/lib/cassav5bt-bluetooth",
  "/var/lib/cassav4-bluetooth"
]);
...
async function assertV6RegistryIsolation(registryValue) {
  const registryPath = path.resolve(registryValue);
  const canonicalCandidate =
    await resolveThroughExistingAncestor(registryPath);
  for (const configuredRoot of FORBIDDEN_LEGACY_STATE_ROOTS) {
    ...
    if (
      ...
      throw cliError(
        ...
        "The V6 CLI cannot access or traverse a legacy device registry"
      );
    }
  }
  try {
    ...
    if (registryStat.nlink !== 1) {
      throw cliError(
        ...
        "The V6 registry must not be a hard link"
      );
    }
  } catch (error) {
    if (error?.code !== "ENOENT") throw error;
...
function validateEnrollmentQr(qr) {
  ...
  const tokenPrefix = `c6e${protocolVersion}_`;
  const tokenBytes =
...
async function main() {
  ...
  const registryPath = await assertV6RegistryIsolation(
    requiredOption(
      options,
      "--registry",
