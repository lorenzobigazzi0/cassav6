...
const TOKEN = `c6e2_${Buffer.alloc(32, 0x4a).toString("base64url")}`;
const P256_ORDER =
...
test("v2 matches the shared Android and Node P-256 golden vector", async () => {
  ...
  assert.equal(
    fixture.kind,
    "cassav6.bluetooth.enrollment-v2-p256-golden-vector"
  );
  const request = {
...
test("v2 validation binds every transcript field", () => {
  ...
  for (const mutation of [
    ...
    { token: `c6e2_${Buffer.alloc(32, 0x4b).toString("base64url")}` },
...
test("v2 signature cryptographically binds both algorithm identifiers", () => {
  ...
  const obsoleteProof = Buffer.from([
    "CASSA_V6-BT-ENROLLMENT-PROOF-V2",
    "2",
...
test("v2 committed response recovers only for the identical request within 600 seconds", async (t) => {
  const directory = await mkdtemp(path.join(tmpdir(), "cassav6-v2-recovery-"));
