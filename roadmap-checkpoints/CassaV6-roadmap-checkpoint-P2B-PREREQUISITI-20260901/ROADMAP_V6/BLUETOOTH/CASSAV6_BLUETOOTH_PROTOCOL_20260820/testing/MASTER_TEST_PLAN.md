...
| Registry | token monouso, commit atomico, recovery, permessi e isolamento V6 | PASS locale |
...
node --test tests/run-v6-bench-inventory.test.mjs
node --test tests/verify-v6-advanced-build-consistency.test.mjs
node scripts/verify-v6-advanced-build-consistency.mjs --root .
```
...
Il monitor Raspberry osserva continuativamente `cassav6.service` e
