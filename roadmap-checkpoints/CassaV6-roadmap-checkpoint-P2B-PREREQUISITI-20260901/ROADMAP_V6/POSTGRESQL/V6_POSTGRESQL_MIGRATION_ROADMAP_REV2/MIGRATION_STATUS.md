...
| RET-01 retention | P11 | TODO | Control plane DEV fail-closed e proposte disabilitate; partizionamento mensile audit con ID globale implementato e testato localmente, applicazione Raspberry in attesa di connettivita; le finestre attendono conferma |
...
| P2 Foundation persistence | IN_PROGRESS | No | MIG-020..MIG-025 completate; MIG-026 mantiene 13 policy disabilitate e viste attive; migration 006 di partizionamento audit verde localmente ma non ancora applicata al DEV per Raspberry irraggiungibile; RET-01 resta aperta |
| P2b Decomposizione server.js | IN_PROGRESS | Pilot only | Slice identity inventariato: 7 route gia estratte, 7 `readDb`, 11 `writeDb`, tutte con dipendenze cross-domain dichiarate; gate globale non superato. Vedere `DOCUMENTAZIONE/P2B_IDENTITY_PILOT_20260901.md` |
...
| MIG-026 | IN_PROGRESS | workspace senza Git | `SORGENTE_SISTEMA/cassa-frontend/backend/tests/postgresql-audit-partitioning.test.mjs`; `SORGENTE_SISTEMA/cassa-frontend/backend/db/postgresql/migrations/006_audit_events_partitioned_retention.sql`; `DOCUMENTAZIONE/MIG026_POSTGRESQL_RETENTION_CONTROL_PLANE_20260831.md` | Partizionamento mensile audit e registro ID globale verdi 28/28 localmente; conversione solo a tabella vuota; applicazione DEV sospesa per Raspberry irraggiungibile; 13 policy restano disabilitate e RET-01 resta aperta |
| P2B-ID-01 | IN_PROGRESS | workspace senza Git | `SORGENTE_SISTEMA/cassa-frontend/reports/postgresql-migration/p2b/identity-route-boundaries.csv`; `identity-pilot-baseline-20260901.json`; `DOCUMENTAZIONE/P2B_IDENTITY_PILOT_20260901.md` | Inventario/gate identity verde: 7 route, 2 handler/1.141 righe, 7 read e 11 write globali; P2b.2 gia presente nello slice, P2b.3/P2b.4 aperte; nessun cambio di comportamento o database |
