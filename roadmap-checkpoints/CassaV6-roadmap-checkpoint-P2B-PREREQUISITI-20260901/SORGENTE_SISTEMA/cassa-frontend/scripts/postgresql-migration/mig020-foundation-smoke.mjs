...
async function exerciseRuntimePrivileges() {
  ...
  try {
    ...
    await client.query(`
      INSERT INTO messaging.idempotency_keys(scope, key, request_hash, status)
      VALUES ('mig020', 'request-1', 'sha256:probe', 'PROCESSING')
    `);
    await client.query(`
      UPDATE messaging.idempotency_keys
      SET status = 'SUCCEEDED', response_code = 200, response_json = '{"ok":true}'::jsonb
    ...
    await expectPgCode(
      () => client.query(`
        INSERT INTO messaging.idempotency_keys(scope, key, status)
        VALUES ('mig020', 'request-1', 'PROCESSING')
      `),
      "23505",
    );
