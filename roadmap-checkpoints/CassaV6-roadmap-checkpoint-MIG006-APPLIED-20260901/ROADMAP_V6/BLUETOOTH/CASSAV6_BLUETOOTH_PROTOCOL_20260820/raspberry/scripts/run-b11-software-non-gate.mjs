...
function material(seed, left, right, direction, field, bytes) {
  return createHash("sha256")
    .update(`V6:B11:${seed}:${left}:${right}:${direction}:${field}`, "utf8")
    .digest()
    .subarray(0, bytes);
}

function messageId(seed, scope) {
  return sha256(`V6:B11:MESSAGE:${seed}:${scope}`).slice(0, 32);
}

function sessionId(seed, scope) {
  return `s${sha256(`V6:B11:SESSION:${seed}:${scope}`).slice(0, 31)}`;
}

function deterministicUuid(seed, index) {
  const value = sha256(`V6:B11:UUID:${seed}:${index}`);
  return [
    value.slice(0, 8),
    value.slice(8, 12),
    `4${value.slice(13, 16)}`,
...
function runB6TopologyCoverage(pairs, plan, metrics) {
  ...
  for (let pairIndex = 0; pairIndex < pairs.length; pairIndex += 1) {
    ...
    const leftAlias = sha256(
      `V6:B11:ALIAS:${plan.seed}:${left.index}`
    ).slice(0, 12);
    const rightAlias = sha256(
      `V6:B11:ALIAS:${plan.seed}:${right.index}`
    ).slice(0, 12);
...
function createChannelPair(input) {
  ...
  const leftChannel = new ReliableChannelV1({
    ...
    peerTrustId: createHash("sha256")
      .update("CASSA_V6-B11-SYNTHETIC-PEER-V1\0", "utf8")
...
function payloadFor(seed, pairIndex, cycle) {
  ...
  const body = createHash("sha512")
    .update(`V6:B11:PAYLOAD:${seed}:${pairIndex}:${cycle}`, "utf8")
    .digest();
...
function buildReport(plan, metrics, leakSnapshot) {
  ...
  const reportWithoutDigest = {
    ...
    seedCommitment: sha256(`V6:B11:SEED:${plan.seed}`),
    topology: {
...
function buildHybridReport(plan, metrics, leakSnapshot, businessWorkload) {
  ...
  const reportWithoutDigest = {
    ...
    timeBasis: "VIRTUAL_MONOTONIC",
    seedCommitment: sha256(`V6:B11:HYBRID:SEED:${plan.seed}`),
    actors: {
...
async function runB11HybridSoftwareNonGate(plan) {
  const root = await mkdtemp(path.join(os.tmpdir(), "v6-b11-hybrid-non-gate-"));
...
export async function runB11SoftwareNonGate(options = {}) {
  ...
  const root = await mkdtemp(path.join(os.tmpdir(), "v6-b11-non-gate-"));
  chmodSync(root, 0o700);
  const clock = { value: START_EPOCH_MS };
  const metrics = {
    roleElections: 0,
