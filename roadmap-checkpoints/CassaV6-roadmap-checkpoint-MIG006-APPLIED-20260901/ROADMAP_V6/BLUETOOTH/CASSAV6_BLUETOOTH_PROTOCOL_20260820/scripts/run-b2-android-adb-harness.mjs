...
const TOKEN_PATTERN = /^c6e1_[A-Za-z0-9_-]{42}[AEIMQUYcgkosw048]$/;
...
export function parseDiscoveryStatus(raw) {
  ...
  if (
    ...
    value.source !== "V6_ANDROID_DISCOVERY_LAB" ||
    value.labBuild !== true ||
...
export function assertReportContainsNoEnrollmentToken(report, token = null) {
  ...
  if (
    ...
    /c6e1_[A-Za-z0-9_-]{43}/.test(serialized) ||
...
export function runSelfTest() {
  const token = `c6e1_${Buffer.alloc(32, 0x4a).toString("base64url")}`;
  const qrBytes = Buffer.from(
    JSON.stringify({
      version: 1,
      enrollmentEndpointId: "v6-lab",
      token
    }),
    "utf8"
  );
  ...
  assert.equal(qr.enrollmentEndpointId, "v6-lab");
...
function usage() {
  return `V6 B2 Android ADB harness
